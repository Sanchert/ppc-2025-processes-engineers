...
## 1. Введение
Сортировка является одной из фундаментальных задач в вычислительной технике. В данной работе рассматривается реализация поразрядной сортировки для чисел с плавающей точкой типа `double` с использованием технологии MPI для параллельной обработки.
...
**Входные данные:**
- Вектор вещественных чисел типа `double` произвольной длины `n`
...
1. Преобразование `double` -> `uint64_t`: Для корректной сортировки отрицательных чисел используется специальное преобразование:
    - Для отрицательных чисел (бит знака = 1) инвертируем все биты
    - Для положительных чисел (бит знака = 0) устанавливаем бит знака в 1
...
- Для каждой позиции (0-7, 8-15, ..., 56-63 биты):
    ...
    - Обратное преобразование `uint64_t` -> `double`: выполняется обратное преобразование для получения исходных чисел

### 3.2 Псевдокод алгоритма
```cpp
void RadixSortDoubles(vector<double>& data) {
    // 1. double -> uint64_t
    vector<uint64_t> keys = transform_doubles_to_keys(data);

    // 2. сортировка
    for (int shift = 0; shift < 64; shift += 8) {
        counting_sort_by_byte(keys, shift);
    }
...
Параллельная реализация использует следующие стратегии:
...
- Данные
    - Размер данных: 100000 элементов для тестов производительности
...
for (size_t i = 0; i < 100000; ++i) {
    double value = static_cast<double>(100000 - i);
    if (i % 3 == 0) {
        value = -value;
    }
    if (i % 7 == 0) {
...
- Общий тест: `{3.5, -2.1, 0.0, 1.1, -3.3, 2.2, -1.4, 5.6}` -> `{-3.3, -2.1, -1.4, 0.0, 1.1, 2.2, 3.5, 5.6}`
...
Таблица времени выполнения (100000 элементов)
...
### 8. Выводы
- **Корректная реализация**: алгоритм корректно обрабатывает все случаи, включая отрицательные числа
